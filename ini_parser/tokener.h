    ...
    */
    ...
    public:
        ...
        */
        ...
        std::string nextString( const std::string& quote );
    ...
    inline std::string Tokener::nextString( const std::string& quote )
    {
        const char* startpos = m_pCurPos;
        const size_t len = quote.length();
        assert( len > 0 );
        ...
        for( size_t i = 0; i < len; ++i )
        {
            ...
            if ( isEnd() )
            {
                if( std::string( m_pCurPos - len, len ) != quote )
                    m_pCurPos = startpos;
                return std::string();
            }
        }

        while ( std::string( m_pCurPos - len, len ) != quote )
        {
            if ( isEnd() )
            {
                m_pCurPos = startpos;
        ...
        assert( m_pCurPos >= startpos + len );
        return std::string( startpos, m_pCurPos - startpos - len );
    }
